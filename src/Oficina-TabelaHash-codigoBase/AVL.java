...
public class AVL<K, V> extends ABB<K, V> {
    ...
    */
		...
		if (raizArvore != null) {
			...
			if (fatorBalanceamento == 2) {
				...
				if (raizArvore.getEsquerda() == null) {
					System.err.println("Aviso: Fator de balanceamento inconsistente - sub-árvore esquerda nula");
				...
				if (raizArvore.getDireita() == null) {
					System.err.println("Aviso: Fator de balanceamento inconsistente - sub-árvore direita nula");
					raizArvore.setAltura();
					return raizArvore;
				}
	...
	private No<K, V> rotacionarDireita(No<K, V> p) {
		...
		if (p == null || p.getEsquerda() == null) {
			throw new IllegalStateException("Rotação direita requer nó e sub-árvore esquerda não nulos!");
		}
	...
	private No<K, V> rotacionarEsquerda(No<K, V> p) {
		...
		if (p == null || p.getDireita() == null) {
			throw new IllegalStateException("Rotação esquerda requer nó e sub-árvore direita não nulos!");
		}
