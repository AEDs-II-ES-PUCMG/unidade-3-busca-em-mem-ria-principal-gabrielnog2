...
public class ABB<K, V> implements IMapeamento<K, V>{
	...
	public int inserir(K chave, V item) {
        if (chave == null || item == null) {
            throw new IllegalArgumentException("Chave e item não podem ser nulos!");
        }
        comparacoes = 0;
        LocalDateTime inicio = LocalDateTime.now();
        try {
    ...
    public Lista<V> recortar(K chaveInicio, K chaveFinal){
        if (chaveInicio == null || chaveFinal == null) {
            throw new IllegalArgumentException("Chaves de recorte não podem ser nulas!");
        }
        
        if (comparador.compare(chaveInicio, chaveFinal) > 0) {
            throw new IllegalArgumentException("Chave de início deve ser menor ou igual à chave final!");
        }
    ...
    private void recortarRecursivo(No<K,V> raizSubArvore, K chaveInicio, K chaveFinal, Lista<V> listaRecorte){
        ...
        if( comparador.compare(chaveInicio, raizSubArvore.getChave()) > 0)
            ...
            try {
                ...
                System.err.println("Erro ao inserir item na lista de recorte: " + e.getMessage());
            }
            recortarRecursivo(raizSubArvore.getDireita(), chaveInicio, chaveFinal, listaRecorte);
        }
    }
    ...
    private void removerAntecessor(No<K,V> raizSubArvore){
        ...
        if (raizSubArvore.getEsquerda() == null) {
            throw new IllegalStateException("Nó não possui sub-árvore esquerda para remoção!");
        }
        ...
        raizSubArvore.setEsquerda(remover(raizSubArvore.getEsquerda(), antecessor.getChave()));
    }
